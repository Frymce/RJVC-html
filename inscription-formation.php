...
<html class="dark" lang="fr">
  <head>
    ...
    <title>Inscription Formation - RJVC Academy</title>
    ...
    <div class="relative flex min-h-screen w-full flex-col items-center overflow-x-hidden">
      <header class="w-full max-w-6xl px-4 py-5">
        <div class="flex items-center justify-between whitespace-nowrap border-b border-solid border-white/10 px-6 sm:px-10 py-3">
          <div class="flex items-center gap-4 text-white">
            ...
            <span class="text-xl font-semibold">RJVC Academy</span>
          </div>
      ...
      <main class="flex flex-col items-center gap-8 px-4 py-8 w-full max-w-4xl">
        <div class="text-center">
          <h1 class="text-4xl font-bold text-white mb-4">Inscription Formation</h1>
          <p class="text-lg text-white/80">Rejoignez RJVC Academy pour développer vos talents</p>
        </div>
        ...
        <form id="formulaire-formation" method="POST" action="process_inscription_formation.php" class="w-full bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10">
          ...
          <div class="mb-8">
            <h2 class="text-2xl font-semibold text-white mb-6">Détails de la formation</h2>
            <div class="space-y-6">
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Formation souhaitée *</label>
                <select name="formation_choisie" required
                  class="form-select flex h-14 w-full rounded-md border border-white/20 bg-white/5 p-[15px] text-white focus:border-primary focus:outline-0 focus:ring-2 focus:ring-primary/40 appearance-none">
                  <option value="">Choisir une formation</option>
                  <option value="developpement" <?php echo (($_SESSION['form_data']['formation_choisie'] ?? '') === 'developpement') ? 'selected' : ''; ?>>Développement IT</option>
                  <option value="design" <?php echo (($_SESSION['form_data']['formation_choisie'] ?? '') === 'design') ? 'selected' : ''; ?>>Design Graphique</option>
                  <option value="photo" <?php echo (($_SESSION['form_data']['formation_choisie'] ?? '') === 'photo') ? 'selected' : ''; ?>>Photographie</option>
                  <option value="video" <?php echo (($_SESSION['form_data']['formation_choisie'] ?? '') === 'video') ? 'selected' : ''; ?>>Vidéographie</option>
                  <option value="leadership" <?php echo (($_SESSION['form_data']['formation_choisie'] ?? '') === 'leadership') ? 'selected' : ''; ?>>Leadership Chrétien</option>
                  <option value="theologie" <?php echo (($_SESSION['form_data']['formation_choisie'] ?? '') === 'theologie') ? 'selected' : ''; ?>>Théologie Pratique</option>
                </select>
              </div>
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Niveau actuel</label>
                <input type="text" name="niveau_actuel" placeholder="Débutant, Intermédiaire, Avancé"
                  class="form-input flex h-14 w-full rounded-md border border-white/20 bg-white/5 p-[15px] text-white placeholder:text-white/40 focus:border-primary focus:outline-0 focus:ring-2 focus:ring-primary/40"
                  value="<?php echo htmlspecialchars($_SESSION['form_data']['niveau_actuel'] ?? ''); ?>">
              </div>
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Objectifs personnels</label>
                <textarea name="objectifs" rows="4" placeholder="Qu'espérez-vous accomplir avec cette formation ?"
                  class="form-input flex w-full rounded-md border border-white/20 bg-white/5 p-[15px] text-white placeholder:text-white/40 focus:border-primary focus:outline-0 focus:ring-2 focus:ring-primary/40"><?php echo htmlspecialchars($_SESSION['form_data']['objectifs'] ?? ''); ?></textarea>
              </div>
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Expérience préalable</label>
                <textarea name="experience_prealable" rows="4" placeholder="Décrivez votre expérience pertinente..."
                  class="form-input flex w-full rounded-md border border-white/20 bg-white/5 p-[15px] text-white placeholder:text-white/40 focus:border-primary focus:outline-0 focus:ring-2 focus:ring-primary/40"><?php echo htmlspecialchars($_SESSION['form_data']['experience_prealable'] ?? ''); ?></textarea>
              </div>
            </div>
          </div>
