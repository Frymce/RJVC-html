...
<html class="dark" lang="fr">
  <head>
    ...
    <title>Participation aux activités - RJVC</title>
    ...
    <div class="relative flex min-h-screen w-full flex-col items-center overflow-x-hidden">
      <header class="w-full max-w-6xl px-4 py-5">
        <div class="flex items-center justify-between whitespace-nowrap border-b border-solid border-white/10 px-6 sm:px-10 py-3">
          <div class="flex items-center gap-4 text-white">
            ...
            <span class="text-xl font-semibold">Participation aux activités</span>
          </div>
      ...
      <main class="flex flex-col items-center gap-8 px-4 py-8 w-full max-w-4xl">
        <div class="text-center">
          <h1 class="text-4xl font-bold text-white mb-4">Rejoignez nos activités</h1>
          <p class="text-lg text-white/80">Participez à la vie communautaire et développez vos talents</p>
        </div>
        ...
        <form id="formulaire-participation" method="POST" action="process_inscription_participation.php" class="w-full bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10">
          ...
          <div class="mb-8">
            <h2 class="text-2xl font-semibold text-white mb-6">Intérêts et disponibilités</h2>
            <div class="space-y-6">
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Intérêts principaux *</label>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <?php 
                  $interets_options = [
                    'worship' => 'Louange & Adoration',
                    'service' => 'Service Communautaire',
                    'study' => 'Étude Biblique',
                    'social' => 'Activités Sociales',
                    'missions' => 'Missions & Évangélisation',
                    'youth' => 'Jeunesse & Sports'
                  ];
                  $saved_interets = $_SESSION['form_data']['interets'] ?? [];
                  foreach ($interets_options as $value => $label): ?>
                  <label class="flex items-center gap-3 p-3 rounded-md border border-white/20 bg-white/5 hover:bg-white/10 transition-colors cursor-pointer">
                    <input type="checkbox" name="interets[]" value="<?php echo $value; ?>"
                      class="form-checkbox h-5 w-5 rounded border-white/30 bg-white/10 text-primary focus:ring-primary/50"
                      <?php echo (is_array($saved_interets) && in_array($value, $saved_interets)) ? 'checked' : ''; ?>>
              ...
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Disponibilités</label>
                <input type="text" name="disponibilites" placeholder="Ex: Weekends, Soirées en semaine..."
                  class="form-input flex h-14 w-full rounded-md border border-white/20 bg-white/5 p-[15px] text-white placeholder:text-white/40 focus:border-primary focus:outline-0 focus:ring-2 focus:ring-primary/40"
                  value="<?php echo htmlspecialchars($_SESSION['form_data']['disponibilites'] ?? ''); ?>">
              </div>
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Compétences</label>
                <textarea name="competences" rows="4" placeholder="Décrivez vos compétences et talents (musique, sport, art, etc.)"
                  class="form-input flex w-full rounded-md border border-white/20 bg-white/5 p-[15px] text-white placeholder:text-white/40 focus:border-primary focus:outline-0 focus:ring-2 focus:ring-primary/40"><?php echo htmlspecialchars($_SESSION['form_data']['competences'] ?? ''); ?></textarea>
              </div>
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Souhaitez-vous faire du bénévolat ?</label>
                <select name="souhaits_benevolat"
                  class="form-select flex h-14 w-full rounded-md border border-white/20 bg-white/5 p-[15px] text-white focus:border-primary focus:outline-0 focus:ring-2 focus:ring-primary/40 appearance-none">
                  <option value="0" <?php echo (($_SESSION['form_data']['souhaits_benevolat'] ?? '') === '0') ? 'selected' : ''; ?>>Non</option>
                  <option value="1" <?php echo (($_SESSION['form_data']['souhaits_benevolat'] ?? '') === '1') ? 'selected' : ''; ?>>Oui</option>
                </select>
              </div>
              <div class="flex flex-col">
                <label class="pb-2 text-base font-medium text-white">Zones d'intérêt spécifiques</label>
                <textarea name="zones_interet" rows="4" placeholder="Domaines spécifiques qui vous intéressent particulièrement"
                  class="form-input flex w-full rounded-md border border-white/20 bg-white/5 p-[15px] text-white placeholder:text-white/40 focus:border-primary focus:outline-0 focus:ring-2 focus:ring-primary/40"><?php echo htmlspecialchars($_SESSION['form_data']['zones_interet'] ?? ''); ?></textarea>
              </div>
            </div>
          </div>
