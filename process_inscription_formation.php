...
// Traitement du formulaire d'inscription formation
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    
    // Vérifier le jeton CSRF
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== ($_SESSION['csrf_token'] ?? '')) {
        $_SESSION['error'] = "Token de sécurité invalide.";
        header('Location: inscription-formation.php');
        exit;
    }
    ...
    // Récupérer les données spécifiques à la formation
    $formation_choisie = $_POST['formation_choisie'] ?? '';
    $niveau_actuel = trim($_POST['niveau_actuel'] ?? '');
    $objectifs = trim($_POST['objectifs'] ?? '');
    $experience_prealable = trim($_POST['experience_prealable'] ?? '');
    ...
    if (empty($formation_choisie)) $errors[] = "Le choix de la formation est obligatoire.";
    ...
    try {
        ...
        // Inscription dans la table spécifique des formations
        $sql_formation = "INSERT INTO inscription_formations 
                         (inscription_id, formation_choisie, niveau_actuel, objectifs, experience_prealable) 
                         VALUES (?, ?, ?, ?, ?)";
